...
uv run --no-sync pytest                          # full: reaches the loading fit
uv run --locked --only-group lint ruff check src/privateassets tests examples docs/conf.py
```

Both environments must be green before a change lands. CI runs the pair on Linux
3.10–3.14 and authoritative Windows/macOS 3.12 cells, and asserts whether
`factorlasso` is importable: a failed optional install otherwise degrades silently
into a green run over two thirds of the suite, which is what happened for several
weeks before 0.6.1.

Line length remains 110 as a per-repository choice. Ruff selects the stack baseline
`E`, `F`, and `W`; `I` remains deliberately absent. Do not reflow the package to
match a sibling.

Tests live in `tests/`. Test data is generated by `tests/synthetic_data.py`, never
downloaded. That module imports
...
- Prefer an enforced invariant to a written convention. The four enforcement
  tests in `tests/test_public_api.py` each caught a real defect.
...
- A release must agree across `pyproject.toml`, `CITATION.cff` and
  `src/privateassets/__init__.py`.
- Repository examples define a `Locals` enum and a `run_local(local=...)` dispatcher, with
  one explicit enum selection in the `__main__` guard.
